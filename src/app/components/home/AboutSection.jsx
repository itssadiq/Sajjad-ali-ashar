...
function LogoWave() {
  return (
    <div className="flex items-center gap-3 text-[#212121]">
      ...
      <span className="font-bold text-[24px] tracking-tight">Logoipsum</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="w-full max-w-360 px-6 sm:px-12 lg:px-24 py-16 flex flex-col justify-between">
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 mb-20 lg:mb-28">
          ...
          <div className="lg:col-span-7 lg:col-start-6 flex flex-col gap-12 lg:gap-16">
            ...
            <div className="flex flex-col items-start gap-8">
              ...
              <button
                type="button"
                className="bg-(--dark-text) text-(--white) hover:bg-(--black) rounded-full px-8 py-4 font-semibold text-[16px] transition-colors duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-black"
              >
                About author
              </button>
            </div>
                ...
                {books.map((book, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="body-longform text-[16px] font-medium text-(--dark-text) group-hover:translate-x-2 transition-transform duration-300">
                        {book.title}
                      </span>
                      {book.badge && (
                        <span className="bg-(--black) text-(--white) text-[12px] font-semibold px-3 py-1 rounded-full whitespace-nowrap leading-none pb-1.25 pt-1.5 group-hover:translate-x-2 transition-transform duration-300">
                          {book.badge}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between w-full sm:w-auto gap-4 mt-2 sm:mt-0 group-hover:-translate-x-2 transition-transform duration-300">
                      ...
                      {/* Arrow indicator */}
                      <svg
                        className="w-5 h-5 text-(--dark-text) transition-transform duration-300 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 sm:block hidden"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
        ...
        {/* Bottom Logos Bar (Marquee Carousel) */}
        <div className="w-full pt-12 pb-6 mt-auto relative overflow-hidden flex">
          {/* Subtle fade effect on left and right */}
          <div className="absolute left-0 top-0 bottom-0 w-16 sm:w-24 lg:w-32 bg-gradient-to-r from-[var(--white)] to-transparent z-10 pointer-events-none"></div>
          <div className="absolute right-0 top-0 bottom-0 w-16 sm:w-24 lg:w-32 bg-gradient-to-l from-[var(--white)] to-transparent z-10 pointer-events-none"></div>
          ...
          `}} />
          <div className="flex animate-marquee w-max min-w-max shrink-0 hover:[animation-play-state:paused]">
            {/* Group 1 */}
            <div className="flex items-center gap-8 lg:gap-12 pr-8 lg:pr-12 flex-shrink-0">
              <LogoWave />
              <LogoFlower />
              <LogoSunburst />
              <LogoPower />
