  ...
  return (
    <header className="w-full flex justify-end items-start z-50 h-10 relative">
      ...
      <button
        ...
        className={`btn-label-sm text-[var(--white)] bg-transparent border border-[#333333] hover:border-[var(--white)] rounded-full px-5 py-2 transition-colors duration-200 cursor-pointer focus:outline-none focus:ring-1 focus:ring-white ${isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'} block`}
      >
        Menu
      </button>
      ...
      {/* Menu Dropdown */}
      {isMenuOpen && (
        <div className="absolute top-0 right-0 bg-[#0a0a0a] border border-[#333333] rounded-2xl p-6 min-w-[240px] shadow-2xl flex flex-col z-50 transition-all duration-300">
          <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={() => setIsMenuOpen(false)} className="text-white hover:text-gray-300 font-bold text-[17px] tracking-wide">Close</button>
          </div>
        </div>
      )}
    </header>
